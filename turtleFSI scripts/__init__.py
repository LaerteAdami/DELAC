...
default_variables = dict(
    ...
    T_control = None) # LA: introduced T_control
...
def merge_xml_files(files):
    ...
    ind = 1 if len(list(last_node)) == 3 else 2 # LA: modified for Warning
    last_timestep = float(last_node[ind].attrib["Value"])

    # Append
    for index in indexes[1:]:
        tree = trees[index]
        for node in list(tree.getroot())[0][0]: # LA: modified for Warning
            ind = 1 if len(list(node)) == 3 else 2 # LA: modified for Warning
